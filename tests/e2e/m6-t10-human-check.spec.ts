...
//   2. 【v1.4】×による解除の出方: 言語別テキストは検索バーと同じ native ×、数値・サムネイル・
//      利用範囲は範囲フィルタと同じ×ボタン、ライセンスは「マスタに従う」の空選択肢。
//      うるさすぎ／気づきにくすぎ でないか。「入力があれば指定・消せばマスタ値」と読めるか
//   2b. 【v1.4】帰属・ライセンス系5欄が加わったフォーム全体の見え方（情報量・並び順）。
//      並び順はマスタ編集フォームに揃えてある
