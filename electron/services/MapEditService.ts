...
class MapEditService {
    ...
    async save(request: MapSaveRequest): Promise<MapSaveResult> {
        ...
        // oct26-m4-t2s（MAJ-1）: v1.0.0 の下書きに残る file://…/{z}/{x}/{y}.<ext> も同じ入口で正規化する
        // （しないと staging と判定されず、Success のままタイル・原本が移らず下書き削除で失われる）
        const url_ = normalizeLegacyTileUrl(mapObject.url_ as string | undefined);
