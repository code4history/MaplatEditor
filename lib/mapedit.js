...
var mapedit = {
    ...
    request: function(mapID) {
        ...
        var loadData = function(data) {
            ...
            if (json.url) {
                ...
                if (json.compiled) {
                    json.compiled = tinObject.setCompiled(json.compiled);
            ...
            new Promise(function(resolve, reject) {
                if (tmpCheck) {
                    ...
                    fs.move(tmpFolder, newTile, function(err) {
                        ...
                        fs.move(newTile + path.sep + 'original.jpg', newOriginal ,function(err){
                            if (err) reject(err);
                            resolve();
                        });
                    });
    ...
    updateTin: function(gcps, strict, vertex) {
        tinObject.setStrictMode(strict);
        tinObject.setVertexMode(vertex);
        tinObject.setPoints(gcps);
        if (gcps.length < 3) {
            focused.webContents.send('updatedTin', 'tooLessGcps');
            return;
        }
        tinObject.updateTinAsync()
            .then(function() {
