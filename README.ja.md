...
- 地図画像の取り込みには JPEG の展開上限がある（既定はメモリ 8192 MB /
  解像度 800 MP）。上限を超える画像は取り込み前に判定され、**必要量・現在の
  設定値・設定すべき推奨値を示すエラー**が表示される。設定ページの「JPEG
  展開メモリ上限」「JPEG 解像度上限」で引き上げられる。これは伸長時の
  暴走を防ぐための算術的なしきい値であり、**実際のメモリを予約する値では
  ない**（引き上げても物理メモリが足りなければ取り込みは失敗する）
...
- 地図は ZIP として書き出され、地図定義・タイル・通常/512px サムネイルに
  加え、POI レイヤを `pois/*.geojson` として参照画像とともに同梱する。
  この ZIP を取り込むと、それらのレイヤは管理下の POI ソースとして復元
  される
