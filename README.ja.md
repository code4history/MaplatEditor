...
- 地図・アプリ内に定義された旧形式の inline POI（pois 配列の埋め込み要素）
  はそのまま保全され、POI データタブに項目数つきの「外部データ」カードとし
  て表示。明示操作「GeoJSONへ変換」で元データを変更せずに編集可能な POI
  ソースの下書きへ変換できる。外部データが残っている間は同じ一覧に
  GeoJSON POI ソースを追加できない（確認つき削除、または変換してから削除）
...
- 編集内容は数秒間隔でローカル下書きとして自動保存され、再度開いたときに
  復元される。保存時に新しいリビジョンと競合した場合はダイアログで解決
  できる
- 地図・アプリ・POI・ベースマップ・アセットはすべて「保存するまで
  作成されない」という同じ挙動に統一されており、破棄された新規下書きは
  ディスクに書き込まれず、一覧に「下書きカード」として表示され再開または
  破棄できる
- 地図の削除は即時削除ではなく trash への退避になる
- アップグレード後の初回起動時、データフォルダを現行形式へ自動移行する
  （レガシーデータ取込・サムネイル生成・trash整合・原本のUUID化）。既存
...
> trash整合・原本のUUIDファイル名への変更）。既存の地図データが削除され
...
- 対応点エラーが未解消の地図はプレビューできません（地図自体の書き出しは
  ...
  できません。
- 削除した地図は trash へ退避されますが、trash の自動削除（purge）は行わ
  れません。手動でクリアしない限りディスク使用量は増え続けます。
