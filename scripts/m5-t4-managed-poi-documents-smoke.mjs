// m5-t4: 複数 managed entry の読み取り API（タスク設計 v2.1 §5.3）。
//
// 固定する受け入れ条件:
//   AC8   importManagedPoiDocuments の契約
...
//   AC9   importPoiZip の無変更（1件制約・戻り値の形）。かつ委譲の前に
//         assertSafePoiPackageEntries を全 entry へ直接呼ぶ
//   AC2   経路レベルの容量上限 — 512 entry 超のタイルを持つ地図 ZIP が読める /
//         payload 超過は拒否される（契約レベルは AC1 が担う）
...
try {
  ...
  await writeFile(entryFile, `
    ...
    // -----------------------------------------------------------------------
    // AC8 (a)(c)(e): dests のものだけ読む / imgs は全 document 横断で1つに畳む /
    //                 poi_sources は作らない
    // -----------------------------------------------------------------------
    {
      ...
      // (a) 未参照 entry を読まない
      assert.equal(r.documents.size, 2, 'AC8(a): dests の2件だけが読まれること');
      assert.equal(r.documents.has('pois/unused.geojson'), false, 'AC8(a): 未参照 entry を読まないこと');

      // (c) 同じ imgs/ を参照する2 document で asset は1つ
      assert.equal(r.createdAssetUids.length, 1,
        'AC8(c): 共有画像は1つの asset へ畳まれること（実際: ' + JSON.stringify(r.createdAssetUids) + '）');
      ...
      assert.equal(await countPoiSources(), before, 'AC8(e): 本 API は poi_sources を作らないこと');

      // cleanup は全 document 分をまとめた1つ
      const residue = await r.cleanup();
      assert.deepEqual(residue, [], 'cleanup が完全補償で空配列を返すこと');
      console.log('ok: AC8 (a)(c)(e)');
    }

    // -----------------------------------------------------------------------
    // AC8 (d): dests にあるのに ZIP に無い → Error（黙って落とさない）
    // -----------------------------------------------------------------------
    {
      const zip = new AdmZip();
      ...
      await assert.rejects(
        ...
        /pois\\/ghost\\.geojson/,
        'AC8(d): 欠損 dest は Error になり、どの dest かが分かること',
      );
      console.log('ok: AC8 (d) 欠損 dest は Error');
    }

    // -----------------------------------------------------------------------
    // AC8 (b): 安全検証は **絞り込み前に ZIP 全体へ**。
    ...
    // -----------------------------------------------------------------------
    {
      ...
      assert.match(source, /assertSafeArchiveEntries\\(/,
        'AC8(b): importManagedPoiDocuments が (a) を呼ぶこと');
      assert.match(source, /assertPoiPayloadLimits\\(/,
        'AC8: (b) を payload へ絞って呼ぶこと');
      assert.match(source, /isPoiPayloadEntry/,
        'AC8: 絞り込みは共有述語 isPoiPayloadEntry を使うこと（別実装を持たない）');
      console.log('ok: AC8 (b) 安全検証の呼び出し');
    }

    // -----------------------------------------------------------------------
    // AC9: importPoiZip の無変更（1件制約・戻り値の形）。
    ...
    // -----------------------------------------------------------------------
    {
      ...
      assert.equal(typeof one.fc, 'object', 'AC9: fc を返すこと');
      assert.equal((one.fc as any).type, 'FeatureCollection');
      assert.ok(Array.isArray(one.createdAssetUids), 'AC9: createdAssetUids を返すこと');
      assert.equal(typeof one.cleanup, 'function', 'AC9: cleanup を返すこと');
      assert.equal(Object.keys(one).sort().join(','), 'cleanup,createdAssetUids,fc',
        'AC9: 戻り値のキー集合が変わらないこと');
      await one.cleanup();

      // 1件制約: 0件 / 2件で throw（POI 単体パッケージの入力検証を緩めない）
      const zeroPath = await mk('poi-zero.zip', (z) => z.addFile('README', Buffer.from('x')));
      await assert.rejects(() => importPoiZip(zeroPath), /found 0/, 'AC9: 0件で throw');
      ...
      await assert.rejects(() => importPoiZip(twoPath), /found 2/, 'AC9: 2件で throw');
    ...
    // -----------------------------------------------------------------------
    {
      ...
      assert.equal(r.documents.size, 1,
        'AC2 経路: 512 entry 超のタイルを持つ地図 ZIP でも managed POI を読めること');
      await r.cleanup();
      ...
      await assert.rejects(
        ...
        'AC2 経路: payload 側（imgs/ 1件 20 MiB 超）は拒否されること',
      );
      console.log('ok: AC2 経路レベル（タイルは無制限 / payload は上限）');
    }
