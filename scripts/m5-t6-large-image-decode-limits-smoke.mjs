...
// Buffer（V8 ヒープ外）である。
//
// 実行時間: タイル生成が支配的である。m19-t6（タイル生成ループの原寸クローン除去）以後の
// 実測は、タイル化が 48 MP 約 29.7 秒 / 110 MP 約 57.1 秒、smoke 全体で約 126 秒。
// m5-t6 時点の記録は 48 MP 約 65 秒 / 110 MP 約 286 秒（合計 6〜8 分）だったが、実行機・
// 実行時期が異なるため新旧の直接比較には使えない（FUTURE_PLAN.md §6 の「注意」。m19-t6 の
// 受け入れ条件も同一プロセス A/B のほうであって、この値ではない）。
// いずれも本タスクのデコード設定とは独立した imageCutter 側の性質である。
