...
//   AC1: resolveMapListImage は '..' を含む fileKey で saveFolder 外の app://bundle/__local を返さず null
...
//        ※oct26-m4-t2（#105）・oct26-m4-t2ff（renderer と同一 origin 化）で表示用 URL は file:// から app://bundle/__local へ移った。正常系は app://bundle/__local で、
...
//            uid が saveFolder 外へ脱出する場合に app://bundle/__local を返さず null
//   AC5(t7): 共通ヘルパ resolveTileZeroFileUrl が素性 fileKey で app://bundle/__local を返し、脱出 fileKey で null
...
try {
  ...
  await writeFile(
    ...
    `
      ...
      // oct26-m4-t2s: #105 の契約 — saveFolder 配下の表示用 URL は app://bundle/__local で、復号すると実パスに戻る
      const isLocalUrlFor = (url, expectedPath) =>
        typeof url === 'string' && url.startsWith('app://bundle/__local/') && appUrlToLocalPath(url) === expectedPath;
      ...
      assert.equal(ac1Tmbs, null,
        'AC1: fileKey が saveFolder 外へ脱出する tmbs パスは null（app://bundle/__local を返さない）: ' + ac1Tmbs);
      ...
      // (1-c) 正常系（uid が UUID を想定した素性もので tmbs に実体がある）は app://bundle/__local を返す
      ...
      assert.ok(isLocalUrlFor(ac1Ok, nodePath.join(tmbsDir, okUid + '.jpg')),
        'AC1 正常系: 素性 fileKey の tmbs は app://bundle/__local を返す (#105): ' + ac1Ok);
      console.log('ok: AC1 (non-regression) regular tmbs returns app://bundle/__local');
      ...
      // (2-b) 正常系: saveFolder 内の thumbnail は app://bundle/__local を返す（非退行）
      ...
      assert.ok(isLocalUrlFor(ac2Ok, nodePath.join(insideThumbDir, 'ok.png')),
        'AC2 正常系: saveFolder 内 thumbnail は app://bundle/__local を返す (#105): ' + ac2Ok);
      console.log('ok: AC2 (non-regression) saveFolder-inside thumbnail returns app://bundle/__local');
      ...
      // (3-b) 正常系: 素性 mapID の legacy も現行どおり app://bundle/__local を返す（非退行）
      ...
      assert.ok(isLocalUrlFor(ac3Ok, nodePath.join(tmbsDir, okMapID + '_menu.jpg')),
        'AC3 正常系: 素性 mapID の legacy は app://bundle/__local を返す (#105): ' + ac3Ok);
      console.log('ok: AC3 (non-regression) regular legacy path returns app://bundle/__local');
      ...
      // (4-b) 正常系: saveFolder 内の iconSource は app://bundle/__local を返す（非退行）
      ...
      assert.ok(isLocalUrlFor(ac4Ok, nodePath.join(appImgDir, 'appicon.png')),
        'AC4 正常系: saveFolder 内 iconSource は app://bundle/__local を返す (#105): ' + ac4Ok);
      console.log('ok: AC4 (non-regression) saveFolder-inside iconSource returns app://bundle/__local');
      ...
      assert.equal(ac4t7, null,
        'AC4(t7): startFrom 地図の uid が saveFolder 外へ脱出しても app://bundle/__local を返さない: ' + ac4t7);
      console.log('ok: AC4(t7) resolveMapTileByRef path returns null for escaping uid');

      // ---- AC5(t7): 共通ヘルパの両極性（素性 fileKey は app://bundle/__local、脱出 fileKey は null）----
      ...
      assert.ok(isLocalUrlFor(helperOk, nodePath.join(dataDir, 'tiles', tileUid, '0', '0', '0.png')),
        'AC5(t7): 素性 fileKey では tiles の app://bundle/__local を返す (#105): ' + helperOk);
      ...
      // 素性 uid では resolveAppListImage が従来どおり tiles の app://bundle/__local を返す。
      ...
      assert.ok(isLocalUrlFor(ac5t7b, nodePath.join(dataDir, 'tiles', tileUid, '0', '0', '0.png')),
        'AC5(t7)-b: 素性 uid のアプリ一覧画像は tiles の app://bundle/__local を返す（非退行・#105）: ' + ac5t7b);
